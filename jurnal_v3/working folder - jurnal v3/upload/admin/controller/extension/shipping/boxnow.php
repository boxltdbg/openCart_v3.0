...
{
	//https://widget-cdn.boxnow.bg/map-widget/client/v4.js
	private $error = array();
		...
		if (isset($this->request->get['order_id']) && $this->request->get['order_id']) {
			...
			if (isset($response) && $response) {
				...
				if (!isset($response_data['status']) && isset($response_data['id'])) {
					...
					$response_data['status_message'] 	= sprintf('Voucher was not created (Error Code: %s). ' . (!empty($error_codes[$response_data['code']]) ? $error_codes[$response_data['code']] : 'Можете да се свържете с нас на <a href="mailto:integrationsupport@boxnow.bg">integrationsupport@boxnow.bg</a>'), $response_data['code']);
					$response_delivery = $this->model_extension_shipping_boxnow->updateRequest($order, $response_data);
